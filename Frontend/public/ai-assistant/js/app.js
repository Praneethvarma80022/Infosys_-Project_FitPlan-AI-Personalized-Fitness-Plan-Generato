class MentalHealthApp {
    ...
    async processUserMessage(userText, source) {
        ...
        // Offline mode: use client-side responses
        if (this.offlineMode) {
    ...
    getLocalResponse(text) {
        ...
        if (/hello|hi|hey|good morning|good evening/.test(lower)) {
            return `Hello ${this.userName}! I'm glad you reached out. I'm here with you. What would you like to talk about?`;
        }
        if (/overwhelm|too much|can'?t cope/.test(lower)) {
            return "That sounds like a lot to carry. When things pile up, it can help to slow down and choose just one small thing to focus on. Would you like to name the one worry that feels most urgent?";
        }
        if (/stress|burnout|pressure/.test(lower)) {
            return "Stress can be really draining. If you're open to it, let's try a short breathing reset: inhale for 4, hold for 4, exhale for 6, repeat 4 times. Want to try it together?";
        }
        if (/anxious|anxiety|panic|nervous/.test(lower)) {
            return "Feeling anxious can make everything feel louder. A grounding exercise can help: name 3 things you can see, 2 you can touch, and 1 sound you can hear. Would you like to try that now?";
        }
        if (/lonely|alone|isolated/.test(lower)) {
            return "I'm sorry you're feeling lonely. You deserve connection and care. Is there a person or place that usually feels a bit safer or kinder to you?";
        }
        if (/sad|down|depressed|hopeless|low/.test(lower)) {
            return "I'm sorry you're feeling down. It makes sense to feel low sometimes, and you don't have to handle it alone. What has been weighing on you the most lately?";
        }
        if (/tired|exhausted|sleep|fatigue/.test(lower)) {
            return "That sounds exhausting. Rest is just as important as exercise. If you could take just one small step to care for yourself right now, what would feel doable?";
        }
        if (/angry|frustrated|irritated|mad/.test(lower)) {
            return "It sounds like something has been really frustrating. It's completely valid to feel angry. Would you like to talk about what triggered this feeling?";
        }
        if (/happy|good|great|amazing|wonderful/.test(lower)) {
            return `That's wonderful to hear, ${this.userName}! What's been going well for you? Recognizing positive moments can help build resilience.`;
        }
        if (/workout|exercise|gym|fitness|training/.test(lower)) {
            return "Exercise is a great way to support both your body and mind. How are you feeling about your fitness journey? Remember, progress isn't always linear.";
        }
        if (/thank|thanks/.test(lower)) {
            return "You're welcome! I'm always here whenever you need to talk. Take care of yourself.";
        }

        return "Thank you for sharing that with me. I'm here to listen and support you. What part of this feels the hardest right now?";
    }
